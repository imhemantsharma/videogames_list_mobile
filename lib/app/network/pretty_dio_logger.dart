...
class PrettyDioLogger extends Interceptor {
  ...
  void _printPrettyMap(
    ...
    var tabsTmp = tabs;
    final isRoot = tabsTmp == initialTab;
    final initialIndent = _indent(tabsTmp);
    tabsTmp++;
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          logPrint('║${_indent(tabsTmp)} "$key": $value${!isLast ? ',' : ''}');
        } else {
          logPrint('║${_indent(tabsTmp)} "$key": {');
          _printPrettyMap(value, tabs: tabsTmp);
        }
      } else if (value is List) {
        if (compact && _canFlattenList(value)) {
          logPrint('║${_indent(tabsTmp)} "$key": ${value.toString()}');
        } else {
          logPrint('║${_indent(tabsTmp)} "$key": [');
          _printList(value, tabs: tabsTmp);
          logPrint('║${_indent(tabsTmp)} ]${isLast ? '' : ','}');
        }
      } else {
        final msg = value.toString().replaceAll('\n', '');
        final indent = _indent(tabsTmp);
        final linWidth = maxWidth - indent.length;
        if (msg.length + indent.length > linWidth) {
          final lines = (msg.length / linWidth).ceil();
          for (var i = 0; i < lines; ++i) {
            logPrint(
                '║${_indent(tabsTmp)} ${msg.substring(i * linWidth, math.min<int>(i * linWidth + linWidth, msg.length))}');
          }
        } else {
          logPrint('║${_indent(tabsTmp)} "$key": $msg${!isLast ? ',' : ''}');
        }
      }
    });
